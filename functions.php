...
if ( ! function_exists( 'basewp_setup' ) ) :
/**
...
function basewp_setup() {
	...
	register_nav_menus( array(
		'primary' => __( 'Primary Menu', 'basewp' ),
        'footer-menu' => __( 'Footer Menu', 'basewp' ),
	) );
	...
	add_theme_support( 'custom-background', apply_filters( 'basewp_custom_background_args', array(
 ...
 */
function basewp_widgets_init() {
	register_sidebar( array(
		'name'          => __( 'Sidebar', 'basewp' ),
		'id'            => 'sidebar-1',
		'description'   => '',
...
add_action( 'widgets_init', 'basewp_widgets_init' );
...
function basewp_scripts() {
	...
	// add bootstrap CSS
	wp_enqueue_style( 'basewp-styles', get_template_directory_uri() . '/css/bootstrap.min.css', array(), '3.3.4', 'all' );
	
	// add font-awesome for icon fonts
	wp_enqueue_style( 'font-awesome', get_template_directory_uri() . '/css/font-awesome.min.css', array(), '4.3.0', 'all' );

	wp_enqueue_style( 'basewp-style', get_stylesheet_uri() );

	// add bootstrap js and add jquery dependency to array
...
require get_template_directory() . '/inc/basewp-options.php';